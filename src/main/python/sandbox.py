...
links = [
    ...
    Link(10, 23944, 1333943632.0, 188,
         "The LOVE game framework versiaon 0.8.0 has been released - with GLSL shader support!",
...
def encrypt(text):
   ...
   for character in text:
      for index, letter in enumerate(alphabet):
         if letter == character:
            oldLetter = letter
            letter = alphabet[(index + (len(alphabet) / 2)) % (len(alphabet))]
            print "Changed letter: ", oldLetter, " to: ", letter
            returnString += letter
   return returnString

def sortBySubmittedTime():
   user_links = []
   for link in links:
      if link.submitter_id == 62443:
            user_links.append(link)
   return_links = sorted(user_links, key=attrgetter('submitted_time'), reverse=False)
   return return_links
...
print encrypt("aaabbbccc")
response = encrypt("aaabbbccc")
